...
<html>
	...
	<body background="img/000.jpg" style="background-repeat: no-repeat;background-size:cover">
        ...
        <div class="box" align="center">
        <h2 style="color:blue">搜 索 结 果</h2>
		</div>
		<div class="box" align="center">
		...
		$sql = "SELECT * FROM report WHERE (timestamp LIKE '%$keywords%' or sensorid LIKE '%$keywords%' or sensorgroup LIKE '%$keywords%') ORDER BY timestamp DESC";
		$res = mysql_query($sql);
		...
		<div class="box" align="center">
		<h2 style="color:blue">采 集 数 据</h2>
		<div class="table1" align="center" style="background-color:white;width:65%;">
		<?php
		error_reporting(0);
        session_start();
		...
		echo "<table border='1'><tr><td>采集时间</td><td>传感器采集批次</td><td>传感器id</td><td>传感器组别</td><td>传感器值</td><td>传感器错误类型</td><td>负责人</td>";
		$sql = "SELECT * FROM sample WHERE (timestamp LIKE '%$keywords%' or sensorid LIKE '%$keywords%' or sensorgroup LIKE '%$keywords%') ORDER BY timestamp DESC";
		$res = mysql_query($sql);
        while($colum = mysql_fetch_array($res)){
			echo "<tr><td>{$colum['timestamp']}</td>
			<td>{$colum['samplecode']}</td>
